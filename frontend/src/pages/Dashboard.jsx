...
export default function Dashboard() {
  ...
  const [stats, setStats] = useState([
    ...
    { label: "Conversion Rate", value: "0%", icon: "./src/assets/conversion.png" },
  ]);
  ...
  // Generate past 7 day labels ending today
  const getPast7Days = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        setStats([
          { label: "Unassigned Leads", value: data.unassignedLeads, icon: "./src/assets/cash.png" },
          { label: "Assigned This Week", value: data.assignedThisWeek, icon: "./src/assets/person.png" },
          { label: "Active Salespeople", value: data.activeSalespeople, icon: "./src/assets/sales.png" },
          { label: "Conversion Rate", value: data.conversionRate, icon: "./src/assets/conversion.png" },
        ]);
          ...
          datasets: [
            {
              ...
              borderRadius: 4,
              barThickness: 24, // Smaller bar width
            },
          ],
        });
      } catch (err) {
                ...
                options={{
                  ...
                  plugins: {
                    ...
                    tooltip: {
                      callbacks: {
                        label: (ctx) =>
                          ctx.raw === null ? "No Data" : `Conversion: ${ctx.raw}%`,
                      },
                    },
                  },
                  scales: {
              ...
              <tbody>
                {employees.map((emp, i) => (
                  <tr key={i}>
                    ...
                    <td>{emp.closed}</td>
                    <td>
                      <span className={emp.status === "Active" ? styles.activeBadge : styles.inactiveBadge}>
                        ● {emp.status}
                      </span>
                    </td>
                  </tr>

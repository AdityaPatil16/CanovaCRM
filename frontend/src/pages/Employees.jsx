    ...
    fetch("${import.meta.env.VITE_API_BASE_URL}/api/employees/stats")
      ...
      .then((data) => {
        const enhanced = data.map((emp) => ({
          ...emp,
          employeeId: emp.employeeId || `#${Math.random().toString(36).substr(2, 8).toUpperCase()}`,
          assignedLeads: emp.assignedLeads ?? 0,
          closedLeads: emp.closedLeads ?? 0,
          status: emp.status ?? "Inactive", // ✅ FIXED HERE
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <aside className={styles.sidebar}>
        <h2 className={styles.logo}>Canova<span className={styles.crmColor}>CRM</span></h2>
      ...
      <main className={styles.mainContent}>
        ...
        <div className={styles.pageHeader}>
          ...
          <button className={styles.addBtn} onClick={() => setShowModal(true)}>Add Employees</button>
        </div>
        ...
        <table className={styles.employeeTable}>
          ...
          <tbody>
            {currentEmployees.length > 0 ? (
                ...
                <tr key={emp._id}>
                  ...
                  <td>
                    <div className={styles.userCell}>
                      {emp.avatar ? (
                        <img src={emp.avatar} className={styles.avatar} alt="avatar" />
                      ) : (
                  ...
                  <td>
                    <span className={`${styles.status} ${emp.status === 'Active' ? styles.active : styles.inactive}`}>
                      {emp.status}
                    </span>
                  </td>
                  <td>
                    <div className={styles.menuWrapper}>
                      <button className={styles.actionBtn} onClick={() => setShowMenuId(emp._id)}>⋮</button>
                      {showMenuId === emp._id && (
              ...
              <tr><td colSpan="7" className={styles.noData}>No employees found.</td></tr>
            )}
          </tbody>
        </table>
        ...
        {totalPages > 1 && (
          <div className={styles.pagination}>
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((num) => (
              <button key={num} onClick={() => paginate(num)} className={currentPage === num ? styles.currentPage : styles.pageNumberBtn}>{num}</button>
            ))}
        ...
        <div className={styles.modalOverlay}>
          <div className={styles.modalContent}>
            <div className={styles.modalHeader}>
              ...
              <button onClick={() => setShowModal(false)} className={styles.closeModalBtn}><IoCloseOutline size={22} /></button>
            </div>
            <div className={styles.modalBody}>
              <input name="firstName" placeholder="First name" value={newEmployee.firstName} onChange={handleInputChange} />
              <input name="lastName" placeholder="Last name" value={newEmployee.lastName} onChange={handleInputChange} />
              <input name="email" placeholder="Email" value={newEmployee.email} onChange={handleInputChange} />
              <select name="location" value={newEmployee.location} onChange={handleInputChange}>
...
<select name="language" value={newEmployee.language} onChange={handleInputChange}>
      ...
      {editModal && selectedEmployee && (
        <div className={styles.modalOverlay}>
          <div className={styles.modalContent}>
            <div className={styles.modalHeader}>
              <h3>Edit Employee</h3>
              <button onClick={() => setEditModal(false)} className={styles.closeModalBtn}><IoCloseOutline size={22} /></button>
            </div>
            <div className={styles.modalBody}>
              <input name="firstName" value={selectedEmployee.firstName} onChange={handleEditChange} />
              <input name="lastName" value={selectedEmployee.lastName} onChange={handleEditChange} />
              <input name="email" value={selectedEmployee.email} disabled />
              <input name="location" value={selectedEmployee.location} disabled />
              <input name="language" value={selectedEmployee.language} disabled />
              <button className={styles.saveBtn} onClick={handleEditSave}>Update</button>
            </div>
          </div>
        </div>
      )}

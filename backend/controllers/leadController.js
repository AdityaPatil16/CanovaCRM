...
exports.uploadLeads = async (req, res) => {
  ...
  fs.createReadStream(req.file.path)
    .pipe(csv())
    .on("data", (row) => {
      const phoneValue = phoneKeys.map(k => row[k]?.trim()).find(Boolean) || "";
      const lead = {
        ...
        type: row.type?.trim().toLowerCase() || "warm",
        status: "unassigned",
        uploadedBy: "admin"
      };
    ...
    .on("end", async () => {
      try {
        ...
        res.status(200).json({ message: "Leads uploaded and distributed.", leads: inserted });
      } catch (err) {
        console.error("❌ Insert Error:", err);
        res.status(500).json({ message: "Error saving leads", error: err.message });
      }
    });
};

// ✅ Distribute Leads & Log Activity
const distributeLeads = async () => {
  const employees = await Employee.find();
  const unassignedLeads = await LeadEntry.find({ assignedTo: null });

  for (const lead of unassignedLeads) {
    const fullMatch = employees.find(emp =>
...
exports.updateLeadType = async (req, res) => {
  ...
  try {
    const lead = await LeadEntry.findByIdAndUpdate(req.params.id, { type }, { new: true });
...
exports.updateLeadStatus = async (req, res) => {
  ...
  try {
    const lead = await LeadEntry.findByIdAndUpdate(req.params.id, { status }, { new: true });
    if (!lead) return res.status(404).json({ message: "Lead not found" });
...
exports.scheduleLead = async (req, res) => {
  ...
  try {
    const lead = await LeadEntry.findByIdAndUpdate(req.params.id, { date, time }, { new: true });
    if (!lead) return res.status(404).json({ message: "Lead not found" });
...
// ✅ Fetch Scheduled Leads (used by Schedule page)
...
// ✅ Combined Update: Type, Status, Schedule
exports.updateLeadInfo = async (req, res) => {
  try {
    ...
    const updatedLead = await LeadEntry.findByIdAndUpdate(id, updateFields, { new: true });
